...
uint8 bcursor[] = {	00,00,88,88,88,88,88,88,88,88,
					...
					00,00,00,00,00,00,88,88,88,88,
					00,00,00,88,00,00,00,88,88,88,
					00,00,88,88,88,00,00,00,88,88,
					00,00,88,88,88,00,00,00,88,88,
					88,88,88,88,88,88,00,00,00,88,
...
{
	if(m_cursor.init(10,14,bcursor,88)==0)
	{
...
void GVC::moveCursor(int16 xoff,int16 yoff,int16 * xpos,int16* ypos)
{
	
	if(xpos!=0) *xpos =	m_cursor.m_ccurx+xoff;
	if(ypos!=0) *ypos =	m_cursor.m_ccury+yoff;  
	m_cursor.move(m_cursor.m_ccurx+xoff,m_cursor.m_ccury+yoff,this);

}

int GCursor::init(int16 xsiz,int16 ysiz,uint8* pbits, uint16 tcolor)
{
	m_cursor.m_xsiz = xsiz;
	m_cursor.m_ysiz = ysiz;
	m_cursor.m_pBits = pbits;

	m_screen.m_xsiz = xsiz;
	m_screen.m_ysiz = ysiz;
	m_screen.m_pBits = new char(xsiz*ysiz);
	if(m_screen.m_pBits==0)
		return -1;
	m_tcolor = tcolor;
	return 0;
}

void GCursor::show(GVC * gvc)
{
	show(m_ccurx,m_ccury, gvc);
...
{
	if(m_validCursor)
		gvc->lBlt(m_ccurx,m_ccury, &m_screen, 0xffff);

}
void GCursor::move(uint16 newx,uint16 newy,GVC * gvc)
{
	if(m_validCursor)
		gvc->lBlt(m_ccurx,m_ccury, &m_screen, m_tcolor);
	show(newx, newy, gvc);
}
